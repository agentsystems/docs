...
The [agent-index repository](https://github.com/agentsystems/agent-index) is a decentralized, Git-based registry for agent metadata. Instead of a centralized API:

- Developers manage their own YAML files in `developers/your-username/`
- Pull requests are automatically validated and merged
- Metadata is published to `https://agentsystems.github.io/agent-index/`
- The AgentSystems UI consumes this index for agent discovery
...
github_username: your-username  # Must match folder name
...
- `github_username` (string): Must match folder name
...
# Image repository (if public)
image_repository_url: docker.io/your-username/your-agent:0.1.0
image_repository_access: public  # public or private
...
listing_status: published       # published, unlisted, archived
...
- `listing_status` (string): "published", "unlisted", or "archived"
- `image_repository_url` (string): Docker image URL
- `image_repository_access` (string): "public" or "private"
...
- **Missing required field**: Ensure `name`, `github_username`, `developer`, `version`, `description`, and `model_dependencies` are present
