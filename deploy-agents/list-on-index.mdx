...
# Default metadata (can be overridden per version)
...
versions:
  - version: "0.1.0"            # Semantic version (x.y.z or x.y.z-beta.1), used as container tag
    is_latest: true             # Exactly one version must be true
    listing_status: listed      # Override default if needed
    readiness_level: beta       # experimental, beta, production, deprecated

    model_dependencies:         # Required for each version
      - gemma3:1b

    input_schema:               # Optional: version-specific parameters
      query:
        type: string
        required: true
        description: Your input parameter

    release_notes: |            # Optional: markdown release notes
      ## Initial Release
...
- `versions` (array): Array of version objects (at least one required)

**Required (in each version object)**:
- `version` (string): Semantic version (e.g., "0.1.0" or "1.0.0-beta.1"), used as container image tag
- `is_latest` (boolean): Exactly one version must be true
- `model_dependencies` (array): Required AI models (at least one)

**Optional (root level - defaults for all versions)**:
...
**Optional (version level - can override defaults)**:
- `listing_status` (string): Override default listing for this version
- `readiness_level` (string): "experimental", "beta", "production", or "deprecated"
- `container_image_access` (string): Override default access
- `required_egress` (array): Required network egress URLs
- `input_schema` (object): Input parameter schema for this version
- `description` (string): Version-specific description override
- `capabilities` (array): Version-specific capabilities override
